...
class ClientBase:
    ...
    def login(self):
        ...
        if response.status_code == 200:
            ...
            print(f"登录成功，访问令牌将于 {self.token_expiry} 过期。")
        else:
    ...
    def refresh_access_token(self):
        ...
        if response.status_code == 200:
            ...
            self.token_expiry = datetime.utcfromtimestamp(access_token_expiration)
            self.session.headers.update(self.headers)
            print(f"访问令牌刷新成功，新令牌将于 {self.token_expiry} 过期。")
        elif response.status_code == 401:
            print("刷新令牌已过期，尝试重新登录...")
            try:
                self.login()
            except Exception as e:
                print("重新登录失败，请检查您的凭据。")
                raise
        else:
            print(f"刷新令牌失败，状态码：{response.status_code}，响应：{response.text}")
    ...
    def ensure_token_valid(self):
        ...
        print(f"当前时间：{current_time}，令牌过期时间：{self.token_expiry}，剩余有效期：{time_diff} 秒")
